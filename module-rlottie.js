  ...
  for (var i = 0, f; (f = files[i]); i++) {
    if (f.type.includes('json')) {
      ...
      read.onloadend = function (e) {
        ...
        store.commit('setLayers', RLottieModule.layers.getLayerTree());
      };
      break;
    }
  }
...
function Layers(RLottieModule, jsString) {
  ...
  this.getLayerTree = function () {
    if (!this.layerTree.length) {
      this.getLayerList();
      this.layerTree = [
        { name: this.originLayers.nm, children: [], type: 'root', keypath: '' },
      ];
      for (let keypath in this.layerList) {
        ...
        initLayerTree(
          ...
          split_keypath,
          0,
          split_keypath.length,
          this.layerList[keypath].type,
        );
  ...
  this.movePrev = function () {
    ...
    this.reload();
    this.highlighting(
      this.RLottieModule.keypath +
        (!this.RLottieModule.isSelectAll
          ? this.RLottieModule.keypath
            ? '.**'
            : '**'
          : ''),
  ...
  this.changeTrScale = function (layer, args) {
    if (layer.s) {
      ...
      else layer.s.k = [args.scaleWidth, args.scaleHeight];
    } else {
      layer.s = {
        a: 0,
